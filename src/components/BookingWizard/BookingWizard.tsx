...
interface BookingWizardProps {
  preselectedService?: ServiceName | null;
}

export default function BookingWizard({ preselectedService }: BookingWizardProps) {
  ...
  // Apply pre-selected service from services section
  useEffect(() => {
    if (preselectedService) {
      setState((s) => ({ ...s, service: preselectedService }));
      setStep(1);
    }
  }, [preselectedService]);
  ...
  const validate = useCallback(() => {
    if (step === 1) {
      if (!state.service) { shake(); return false; }
    }
    if (step === 2) {
      if (!state.date || !state.time) { shake(); return false; }
    }
    if (step === 3) {
  ...
  const handleNext = () => {
    if (!validate()) return;
    if (step === 4) {
      if (!state.sms && document.getElementById('bdar-consent')) {
        const cb = document.getElementById('bdar-consent') as HTMLInputElement;
        if (!cb?.checked) {
          setGdprError(true);
      ...
      // Check GDPR consent
      const cb = document.getElementById('bdar-consent') as HTMLInputElement | null;
      if (!cb?.checked) {
        setGdprError(true);
        setTimeout(() => setGdprError(false), 2200);
        return;
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    calDays.push({
      ...
      d,
      date,
      disabled: isPast || isWeekend || isFullyBooked,
      isToday,
      isSelected,
    });
  }

  const unavailSlots = state.date ? (UNAVAIL_BY_DOW[state.date.getDay()] ?? []) : [];
            ...
            {submitted ? (
              <div className={styles.wizardSuccess} aria-live="polite" aria-atomic="true">
                ...
                <div className={styles.wizardSuccessDetail}>
                  ...
                  {state.sms && <><br /><small style={{ color: 'var(--sage)' }}>SMS priminimas įjungtas</small></>}
                </div>
              </div>
            ) : (
              <>
                ...
                {step === 1 && (
                  <div className={styles.wizardPanel} role="group" aria-labelledby="step1-title">
                    ...
                    <div className={styles.svcGrid} role="radiogroup" aria-label="Paslaugų pasirinkimas">
                      {SERVICES.map((svc) => {
                        const isSelected = state.service === svc.name;
                        return (
                          <div
                            key={svc.id}
                            ...
                            tabIndex={0}
                            onClick={() => { setState((s) => ({ ...s, service: svc.name })); setStep(2); wizardRef.current?.scrollIntoView({ behavior: 'smooth', block: 'nearest' }); }}
                            onKeyDown={(e) => {
                              if (e.key === 'Enter' || e.key === ' ') {
                                e.preventDefault();
                                setState((s) => ({ ...s, service: svc.name }));
                                setStep(2);
                                wizardRef.current?.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
                              }
                            }}
                ...
                {step === 2 && (
                  <div className={styles.wizardPanel} role="group" aria-labelledby="step2-title">
                    ...
                    <div className={styles.wizardPanelSub}>Laisvi laikai rodomi pasirinkus datą. Savaitgaliai nedarbo dienos.</div>
                      ...
                      <div>
                        <div className={styles.cal} aria-label="Datų kalendorius">
                          ...
                          <div className={styles.calDays}>
                            {calDays.map((cell) => {
                              if (cell.type === 'empty') {
                                return <div key={cell.key} className={`${styles.calDay} ${styles.empty}`} />;
                              }
                              const cls = [
                                styles.calDay,
                      ...
                      <div className={styles.timeSection}>
                        ...
                        {!state.date ? (
                          ...
                          <div className={styles.timeGrid} role="radiogroup" aria-label="Laiko pasirinkimas">
                            {ALL_SLOTS.map((slot) => {
                              const unavail = unavailSlots.includes(slot);
                              ...
                              return (
                                <div
                                  ...
                                  aria-label={unavail ? `${slot} — užimta` : slot}
                                  tabIndex={unavail ? undefined : 0}
                                  onClick={() => !unavail && pickTime(slot)}
                                  onKeyDown={(e) => {
                  ...
                  <div className={styles.wizardPanel} role="group" aria-labelledby="step3-title">
                    ...
                    <div className={styles.wizFormGroup}>
                      <label htmlFor="wiz-notes">
                        Pastabos <span className={styles.optional}>(neprivaloma)</span>
                      </label>
                      <textarea
                        id="wiz-notes"
                        placeholder="Aprašykite savo situaciją arba užduokite klausimą specialistei..."
                        value={state.notes}
                    ...
                    <div
                      className={`${styles.smsToggle} ${state.sms ? styles.on : ''}`}
                      role="switch"
                      aria-checked={state.sms}
                      tabIndex={0}
                      aria-label="SMS priminimas"
                      onClick={() => setState((s) => ({ ...s, sms: !s.sms }))}
                      onKeyDown={(e) => {
                        if (e.key === 'Enter' || e.key === ' ') {
                          e.preventDefault();
                          setState((s) => ({ ...s, sms: !s.sms }));
                        }
                      }}
                    >
                    ...
                    <div className={styles.confirmSummary}>
                      ...
                      <div className={styles.confirmRows}>
                        {[
                          ...
                          ['SMS', state.sms ? '✓ Įjungtas' : 'Išjungtas'],
                        ].map(([label, value]) => (
