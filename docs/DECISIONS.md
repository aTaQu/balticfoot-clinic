...
- `ClinicSettings.notificationEmails` introduced as a separate list from `ClinicSettings.email`. `email` does double duty as public contact (Footer, privacy policy, patient-facing template footers); conflating it with the owner-alert target means changing the public contact silently re-routes internal alerts. All three owner-facing alerts (`new-booking-alert`, `booking-cancelled-alert`, `contact-enquiry-alert`) now loop over the list. Field is `required: true, minRows: 1` so the admin UI can't enter an "alerts go nowhere" state; existing rows backfilled via `onInit`. Rejected alternative: fan out to all `users` with `role: 'admin'` — would have re-coupled "who can log in" with "who gets pinged", making it impossible to opt one admin out of alert noise or to add a co-owner who doesn't need a Payload login.
- Patient-facing `booking-cancelled` email added with required `cancellationReason`. Rezervacija lifecycle now has parity: rejected (pre-confirmation refusal) and cancelled (post-confirmation tear-down) both communicate a reason. Reason is mandatory at the action layer (validated in `cancelBooking`, surfaced as a required input in `BookingActions`); not at the schema level so historical cancelled rows stay valid. The reason is also written to the AuditLog `note` and included in the owner `booking-cancelled-alert` for cross-owner visibility.

## Availability inversion (Darbo laikai) — implemented 2026-06-25

Grilled, then implemented test-first the same day. Inverts the availability model
from opt-out to opt-in and **supersedes the Phase 16 line above that rebranded
`blocked-slots` as "Nedarbo laikas"**. Glossary updated in `CONTEXT.md`.

- **Why invert.** Most appointments live in the owner's paper notebook, so the website is the *secondary* book. A default-open site would expose slots already taken on paper. The site now defaults to **closed**; the owner publishes the specific windows she wants bookable online. Opt-in availability, not opt-out.
- **Replace, not layer.** Keeping both blocked-slots and open-windows would force two contradictory mental models ("I block busy time" vs "I publish free time"). `blocked-slots` is replaced outright by `availability-windows` (canonical LT label **Darbo laikai**, singular **Darbo laikas**).
- **Per-date windows, no recurrence.** Each window is `date` + `startTime` + `endTime`. A recurrence/exception engine was rejected as overkill for a single-practitioner clinic whose online openings are ad-hoc leftovers; a "repeat for N weeks" convenience action can be added later without changing the model.
- **Windows are the sole booking authority.** `ClinicSettings.workingHoursStart/End` and `openDays` become **advertising-only** (footer "our hours"); they no longer gate availability. Rejected an "outer gate" (windows must fall within working hours/open days) because it would silently swallow legitimate windows (e.g. a Sunday opening) — the same silent-reshaping footgun avoided elsewhere.
- **Clock-grid 15-min slots.** Added `'15'` to `slotIntervalMinutes`. Candidate start times snap to the clock grid (:00/:15/:30/:45); an off-grid open like 14:20 yields its first slot at 14:30. Finer steps are safe — the existing booking-overlap check prunes colliding candidates as bookings come in.
- **Algorithm.** A grid time is bookable iff `[start, start+duration)` fits fully inside an open window AND overlaps no pending/confirmed booking. Only window slots are emitted; the `{ slots: [{ time, available }] }` response contract is unchanged.
- **Naming registers.** Owner/admin term **Darbo laikai**; patient-facing wizard label stays **Laisvi laikai** (already in `BookingWizard.tsx`). Slug `availability-windows`. The "darbo" overload against the advertising `Darbo dienos` / `Darbo pradžia–pabaiga` is accepted and flagged in `CONTEXT.md`.
- **Migration: drop & recreate.** The site is **live**, but blocked↔open is a semantic inversion — an old "blocked" row can't be transformed into an "open" one — so existing `blocked_slots` rows are intentionally discarded, not migrated. Drop `blocked_slots`, create `availability_windows` (`date`, `startTime`, `endTime`, optional `note`, `createdBy`); add `'15'` to the interval enum. **Live cutover is default-closed**: until the owner publishes Darbo laikai windows the booking page offers no online slots (existing bookings untouched). Validated end-to-end via `payload migrate` on a scratch DB; auto-applied on deploy (Dockerfile `CMD` runs `payload migrate && next start`).
- **Audit log untouched.** `slot_blocked`/`slot_unblocked` were never actually written (dead enum values); left as legacy cruft to avoid an enum migration. No auditing of window edits.
- **Downstream.** Dashboard "Savaitės grafikas" shows windows as an available band + bookings, with empty days marked **Uždaryta**. Booking wizard gains a phone CTA on empty dates (now the common case). Footer advertised hours unchanged.
- **Booking calendar ungated** (implementation consequence). `BookingWizard`'s calendar no longer disables non-`openDays` — it only blocks past dates — otherwise a window on a former closed day (e.g. Sunday) would be unreachable. The `openDays` prop was dropped from the wizard (still used by the Footer). Booking submission needed **no** change: `createBooking`'s availability re-check (`bookings.ts`) already rejects any slot outside a window via the inverted `getAvailability`.
- **Migration vs dev push** (implementation note). Dev/test DB syncs via Payload `push`; the rename triggered an interactive "create or rename?" push prompt that hangs headless. Resolved by applying the transition SQL directly for the test loop and committing a hand-written migration (`src/migrations/20260625_120000.ts`) as the prod artifact. `'15'` added to the `slot_interval_minutes` enum. Verified: `tsc --noEmit` clean + full vitest suite 38/38.
- **Calendar shows only bookable days** (follow-up to the ungating). Default-closed makes most dates empty, so a bare past-only-gated calendar would have patients clicking through empties. The wizard now fetches `getAvailableDates` (new `GET /api/availability/dates`, 120-day horizon) and **enables/accents only dates with a free slot**, **auto-jumps to and preselects the next open day**, and shows a phone CTA when there are none. Empty days are disabled (greyed), not clickable — chosen over keeping them clickable-with-CTA for a cleaner "these are the open days" signal. `getAvailableDates` reuses `computeDaySlots` (extracted from `getAvailability`) and is batched to 2 queries over the range.
