...
- **Migration: drop & recreate.** Pre-launch, and an old "blocked" row means the inverse as an "open" row, so nothing is carried over. Drop `blocked_slots`, create `availability_windows` (`date`, `startTime`, `endTime`, optional `note`, `createdBy`); add `'15'` to the interval enum. Generated in the devcontainer only.
